...
public class ApiCodeGenerator
{
    ...
    public TsTypeGenerator TypeGenerator;
    public TsTypeConverterGenerator ConverterGenerator;
    ...
    public void Output(TypeScriptWriter writer)
    {
        ...
        TypeGenerator.OutputImports(imports);
        if (imports.Count > 0)
        {
        ...
        TypeGenerator.OutputTypeDeclarations(writer);
        ...
        ConverterGenerator.OutputTypeConverters(writer);
    }
    ...
    protected void OutputService(TypeScriptWriter writer, ServiceDesc s)
    {
        ...
        using (writer.Indent())
        {
            ...
            foreach (var method in s.Methods.OrderBy(m => m.TsName))
            {
                foreach (var httpMethod in method.HttpMethods.Order())
                {
                    bool canDirectReturn = !ConverterGenerator.NeedsConversion(method.ReturnType);
                    ...
                    using (writer.Indent())
                    {
                        ...
                        foreach (var p in method.Parameters)
                            ConverterGenerator.OutputTypeConversion(writer, p.TsName, p.Type, toTypeScript: false);
                        ...
                        if (!canDirectReturn)
                        {
                            ...
                            ConverterGenerator.OutputTypeConversion(writer, "result", method.ReturnType, toTypeScript: true);
...
#warning TODO: refactor
    ...
    public void OutputTypeDeclarations(TypeScriptWriter writer)
    {
        writer.WriteLine("declare global {");
        writer.WriteLine();
        using (writer.Indent())
        {
            var compositeTypes = Types.OfType<CompositeTypeDesc>();
            var enumTypes = Types.OfType<EnumTypeDesc>();
    ...
    public void OutputImports(HashSet<string> imports)
    {
        foreach (var t in Types.OfType<BasicTypeDesc>())
...
public class TsTypeConverterGenerator
{
    private HashSet<TypeConverter> convertersUsedFrom = new();
    private HashSet<TypeConverter> convertersUsedTo = new();
